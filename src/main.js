...
const { interpolateAs } = require("next/dist/next-server/lib/router/router");
const { isCommunityResourcable } = require("@ethereum-waffle/provider/node_modules/@ethersproject/providers");
...
// connect to the Chainlink VRF contract to get the random attributes
const getRandomNumbers = async(contractAddr) => {
  const RandomCryptoMarc = await hre.ethers.getContractFactory("RandomCryptoMarc")
  //Get signer information
  let provider = hre.ethers.getDefaultProvider('kovan');
  //Create connection to Contract and call the createRequestTo function
  const vrfConsumerContract = new hre.ethers.Contract(contractAddr, RandomCryptoMarc.interface, provider)
  ...
  let jsonRes = {}
  let res = new Array(lastTokenId)
  let res2 = new Array(lastTokenId)
  for (i = 0; i < lastTokenId; i++) {
      res[i] = await vrfConsumerContract.attributes(i+1)
      // console.log('res: ', res[i].toString())
      res2[i] = await vrfConsumerContract.competences(i+1)
      jsonRes = { attributes: res[i], skills: res2[i] }
      randomNumbers[i] = jsonRes
  }  
  // await getSkills("0x5C006617eC722Ff755387CfB4363F720D40afD70", randomNumbers[2].skills[1].toString())
...
const getSkills = async(contractAddr, number) => {
  ...
  const ApiConsumerContract = new hre.ethers.Contract(contractAddr, APIConsumer.interface, provider)
  const apiConsumerContract = ApiConsumerContract.connect(wallet)
  const url = "https://competenties.fnctn.nl/api/OP-" + number
  const path = "essentialCompetences.0.code"
  let tx = await apiConsumerContract.requestVolumeData(url, path, {gasPrice: hre.ethers.utils.parseUnits('100', 'gwei'), gasLimit: 1000000});
  console.log("Tx success hash: ", tx.hash)
  console.log("Sleeping 120s for correct API response..")
  result = sleep(readSkill, apiConsumerContract);
  return result
}

const readSkill =  async(contract) => {
  let result = (await contract.volume()).toString()
  console.log("Skill: ", result)
  return result
}

const getChainlinkSkills = async(contractAddr, skills) => {
  let skillsAPI = []
  for (i = 0; i<skills.length; i++) {
    await getSkills(contractAddr, skills[i])
      .then(data => skillsAPI.push(data))
      .catch(err => console.log("There was an error reading the Chainlink API response.."))
  }
  return skillsAPI
}
...
const startCreating = async () => {
  ...
  while (layerConfigIndex < layerConfigurations.length) {
    ...
    while (
      ...
      traits = randomNumbers[count-1].attributes
      skills = randomNumbers[count-1].skills
      let newDna = createDnaRandom(layers, randomNumbers[count-1].attributes);
...
module.exports = { startCreating, createBreed, getSkills, buildSetup, getElements };
